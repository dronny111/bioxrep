...
- Scaling the fact-disjoint HGVS split changes the result substantially. A new split requiring protein and nucleotide expressions yields `29,076` train classes and `7,270` test classes, with `50,000` train pairs and `20,000` test pairs. The scaled strict-filled hard test set has `2,000` queries with `20` candidates/query and char n-gram top-1 `0.154`, top-5 `0.3995`, MRR `0.296`. A text-only CNN trained on the scaled 50k pair file reaches full 2k strict-filled hard top-1 `0.6995`, top-5 `0.9465`, MRR `0.812`, making it the strongest BioXRep result so far.
- Scaled ablations clarify the role of numeric features. On the same full 2k strict-filled hard benchmark, the sinusoidal text+numeric model reaches old pair top-1 `1.0000` but hard top-1 `0.4875`; numeric-only reaches old pair top-1 `1.0000` but hard top-1 `0.3480`; masked-digit text+numeric reaches hard top-1 `0.5385`. Numeric features produce very high hard top-5, but text-only remains best for hard top-1, so the main table should separate representation learning from numeric-feature upper-bound behavior.
