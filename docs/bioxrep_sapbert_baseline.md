...
| `alias_symbol` heldout | **BioXRep byte-Transformer student** (5-seed mean) | `0.024` | `0.055` | `0.040 ± 0.001` |
...
**Architecture control (byte-Transformer).** To rule out that the negative is an artifact of the convolutional inductive bias, we trained a byte-level Transformer encoder (2 layers, 4 heads, ~133k parameters) from scratch under the *identical* contrastive objective, byte tokenization, and 5-seed protocol as the char-CNN — the only change is the sequence-mixing operator (convolution → self-attention). It does **worse**, not better: held-out `alias_symbol` MRR `0.040 ± 0.001` vs the char-CNN's `0.059 ± 0.001` (a gap of >10 seed standard deviations), and lower in-domain validation top-1 as well (`0.449 ± 0.010` vs `0.598 ± 0.005`). Self-attention over raw bytes is no better than convolution at inducing notation invariance from the contrastive signal alone, and both remain far below the lexical floor. The binding constraint is the absence of a pretrained semantic signal, not the choice of surface encoder — which is exactly why SapBERT (semantically pretrained) is the only method that clears the n-gram baseline on the held-out notation.
