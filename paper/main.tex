...
validation top-1 but only $0.051$ MRR on held-out \texttt{alias\_symbol} queries
...
(in-domain validation top-1 $0.985$) yet fails to transfer: it scores $0.051$ MRR
...
\multirow{5}{*}{\shortstack[l]{\texttt{alias\_}\\\texttt{symbol}\\(held-out)}}
 ...
 & Char-CNN student     & 0.035 & 0.064 & 0.051 \\
...
contest. It scores $0.051$ MRR on \texttt{alias\_symbol}, below SapBERT dense
($0.134$) and even the character $n$-gram floor ($0.077$). Two factors explain the
...
configuration (char-CNN, hidden $64$, projection $128$, $3$ epochs, seed $13$,
same fact-disjoint split), varying only the auxiliary loss that supervises the
...
\midrule
Contrastive only                 & 0.050 & 0.033 & 0.753 \\
\;\; + byte-rule attn.\ distill   & 0.054 & 0.035 & 0.769 \\
\;\; + neural-teacher attn.\ distill & 0.050 & 0.037 & 0.804 \\
\bottomrule
\end{tabular}
\caption{Attention-distillation ablation on held-out \texttt{alias\_symbol}
(same $2{,}000$ queries / $45$k pool as Table~\ref{tab:results}). MRR $95\%$
bootstrap intervals overlap across all three arms
($[0.042,0.058]$, $[0.045,0.062]$, $[0.042,0.059]$). The invariance ratio is the
mean within-class over between-class embedding cosine distance (lower is more
notation-invariant). Neither teacher improves held-out retrieval, and both
\emph{increase} the invariance ratio.}
\label{tab:ablation}
\end{table}

Neither form of attention distillation closes the gap (Table~\ref{tab:ablation}).
The deterministic teacher leaves held-out MRR statistically unchanged
($0.050 \rightarrow 0.054$, overlapping bootstrap intervals); the neural teacher,
despite carrying a demonstrably stronger alignment signal, is indistinguishable
from the contrastive-only baseline ($0.050$) and yields the \emph{worst} invariance
ratio of the three ($0.804$). Distilling a better \emph{surface-form} attention map
does not induce notation invariance here---consistent with the reading that the
limit is representational capacity, not the supervisory target's quality. This is a
...
\section*{Limitations}
The study covers a single entity type (genes, via HGNC) and a single lightweight
student architecture; whether the negative result extends to larger surface-only
encoders or to other entity types (variants, chemicals, phenotypes) is not
established here. Queries are capped at $2{,}000$ per notation for tractable CPU
