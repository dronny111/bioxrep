...
\midrule
\multirow{6}{*}{\shortstack[l]{\texttt{alias\_}\\\texttt{symbol}\\(held-out)}}
 ...
 & Byte-Transformer student$^{\ddagger}$ & 0.024 & 0.055 & 0.040 \\
...
$^{\ddagger}$the held-out \texttt{alias\_symbol} student rows (char-CNN and
byte-Transformer) are the mean over $5$ seeds ($\{13,17,23,42,101\}$; per-seed std
$\le 0.003$ on every metric); the deterministic lexical, SapBERT, and BioSyn
methods are seed-invariant, and the train-seen ($^{\dagger}$) student rows are
single-seed. The byte-Transformer is trained from scratch under the identical
contrastive regime and byte tokenization as the char-CNN, changing only the
sequence-mixing architecture (convolution~$\rightarrow$~self-attention).}
\label{tab:results}
\end{table}
...
The negative is also not an artifact of the convolutional inductive bias. Replacing
the char-CNN's convolutional trunk with a $2$-layer, $4$-head byte-level
Transformer encoder---trained from scratch under the identical contrastive
objective and byte tokenization, so that only the sequence-mixing operator
changes---does not recover the task. The byte-Transformer scores $0.040 \pm 0.001$
MRR on held-out \texttt{alias\_symbol} (mean $\pm$ std over the same $5$ seeds),
\emph{below} the char-CNN ($0.059 \pm 0.001$) by more than ten seed standard
deviations, and its in-domain validation top-1 is likewise lower ($0.449 \pm 0.009$
vs.\ $0.598 \pm 0.004$). Self-attention over raw bytes is thus no better---and here
slightly worse at matched budget---than convolution at inducing notation-invariant
structure from the contrastive signal alone; both remain far below the lexical
floor. The limitation is the absence of a pretrained semantic signal, not the
choice of surface encoder, which is precisely why the semantically pretrained
SapBERT is the only method that clears the $n$-gram baseline on the held-out
notation.
...
mean between-class over within-class embedding cosine distance (higher is more
separated/invariant); the three arms are indistinguishable once seed variance is
