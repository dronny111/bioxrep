...
hard negatives, we find that neither injecting position as a continuous input feature
nor adding an auxiliary numeric-consistency loss improves hard top-1 over a text-only
encoder---the shared position is a confound rather than a discriminator---so the
numeric half inherits the same capacity-limited negative as the symbolic half.
